            ...
            <div class="flex-1">
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    <div class="p-6 border-b border-gray-200">
                        <h2 class="text-xl font-bold text-gray-800">بررسی رزومه: {{ $resumeStorage->first_name . " " . $resumeStorage->last_name }}</h2>
                    </div>
                    <div class="p-6">
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg">
                                ...
                                <div class="space-y-2">
                                    <p class="text-sm text-gray-700">
                                        <span class="font-medium">نام:</span> {{ $resumeStorage->first_name }}
                                    </p>
                                    <p class="text-sm text-gray-700">
                                        <span class="font-medium">نام خانوادگی:</span> {{ $resumeStorage->last_name }}
                                    </p>
                                    <p class="text-sm text-gray-700">
                                        <span class="font-medium">ایمیل:</span> {{ $resumeStorage->email }}
                                    </p>
                                    <p class="text-sm text-gray-700">
                                        <span class="font-medium">شماره تماس:</span> {{ $resumeStorage->mobile }}
                                    </p>
                                    <p class="text-sm text-gray-700">
                                        <span class="font-medium">حقوق پیشنهادی:</span> {{ number_format($resumeStorage->job_position->price) . " تومان" }}
                                    </p>
                                    <p class="text-sm text-gray-700">
                                        <span class="font-medium">نامه همراه:</span>
                                    </p>
                                </div>
                            </div>
                            <div class="bg-gray-50 p-4 rounded-lg relative">
                                <h3 class="text-lg font-bold text-gray-800 mb-3">پیش‌نمایش رزومه</h3>
                                <iframe id="resume-preview" src="{{ $resumeStorage->file_path }}" class="w-full h-96 border border-gray-200 rounded-lg"></iframe>
                                <div class="mt-3 flex justify-end space-x-3 space-x-reverse">
                                    <a href="{{ $resumeStorage->file_path }}" download class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition duration-200">
                                        دانلود رزومه
                                    </a>
                        ...
                        <form method="POST" action="{{ route('panel.job-position.resume.submit_score', [$jobPosition->id, $resumeStorage->id]) }}">
                            @csrf
                            <input type="hidden" name="resume_id" value="{{ $resumeStorage->id }}">
                            <input type="hidden" name="jobPosition_id" value="{{ $jobPosition->id ?? 1 }}">
                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    <label for="score" class="block text-sm font-medium text-gray-700 mb-1">امتیاز فنی (از ۱ تا ۱۰)</label>
                                    <input type="number" id="score" name="score" min="1" max="10" class="w-full px-4 py-2 border border-gray-200 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" value="{{ old('score', $resume_score_info->score ?? '') }}">
                                </div>
                                <div>
                                    <label for="comments" class="block text-sm font-medium text-gray-700 mb-1">توضیحات</label>
                                    <textarea id="comments" name="comments" rows="4" class="w-full px-4 py-2 border border-gray-200 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('comments', $resume_score_info->description ?? '') }}</textarea>
                                </div>
                                <div>
                                    ...
                                    <select id="status" name="status" class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-gray-700 hover:bg-gray-50 transition duration-200 appearance-none text-right pr-10" style="background-image: url('data:image/svg+xml;charset=UTF-8,%3csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 24 24%22 fill=%22none%22 stroke=%22%23666666%22 stroke-width=%222%22 stroke-linecap=%22round%22 stroke-linejoin=%22round%22%3e%3cpolyline points=%226 9 12 15 18 9%22%3e%3c/polyline%3e%3c/svg%3e'); background-position: left 0.5rem center; background-repeat: no-repeat; background-size: 1.5em;">
                                        @foreach($statuses as $status)
                                            <option value="{{ $status->id }}" @if($resumeStorage->status->id == $status->id) selected @endif>{{ __($status->name) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
